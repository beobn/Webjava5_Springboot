...
<div class="row">
    <form:form method="post" action="/admin/cart/getall">
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="idOrder" id="" class="form-control" placeholder=""
...
<table class="table table-striped">
    ...
    <c:forEach items="${page.getContent()}" var="xy">
            ...
            <tr>
                ...
                <td>${xy.emailSend }</td>
                <td>
                    <c:if test="${xy.users==null }">Khách</c:if>
                    <c:if test="${xy.users!=null}">${xy.users.userName }</c:if>
                </td>
                <td>
                        ${xy.quantity }
                </td>

                <td>${xy.priceOrder } VNĐ</td>
                <td>
                            <c:if test="${xy.status=='STT0'}">Lỗi Thanh Toán</c:if>
                            <c:if test="${xy.status=='STT1'}">Đã Thanh Toán</c:if>
                </td>
...
<nav aria-label="Page navigation example">
    <ul class="pagination justify-content-end">
        <li class="page-item <c:if test="${page.getNumber() < 1 }">disabled</c:if> ">
            ...
            <a class="page-link" href="/admin/cart/getall?pageNumber=${page.getNumber() - 1}"> < </a>
