...
<div class="row">
    <form:form method="post" action="/admin/categori/getall">
        <div class="mb-3">
            ...
            <tr>
                <div class="modal fade" id="example${xy.id}" tabindex="-1" aria-labelledby="exampleModalLabel"
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Cập Nhâp Categorie</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                        aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <form:form action="/admin/categori/update?id=${xy.id}" method="post">
                                    <div class="mb-3">
                                        <label for="" class="form-label">Tên Sản Phẩm:</label>
                                        <input type="text" name="name" id="" class="form-control" placeholder=""
                                               value="${xy.name}" aria-describedby="helpId">

                                    </div>
                                    <div class="mb-3">
                                        <label for="" class="form-label">Giá Sản Phẩm:</label>
                                        <input type="number" min="5000" max="1000000000" name="price" id="" class="form-control" placeholder=""
                                               value="${xy.price}" aria-describedby="helpId">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control" name="classify" id="">
                                            <c:forEach items="${listclassify}" var="xx">
                                                <option value="${xx}"
                                                        <c:if test="${xx==xy.classify}">selected</c:if> >
                                                        <%-- ${value}--%>
                                                    <c:if test="${xx=='VT'}">Viettel</c:if>
                                                    <c:if test="${xx=='VN'}">Vina</c:if>
                                                    <c:if test="${xx=='MB'}">Mobi</c:if>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control" name="value" id="">
                                            <c:forEach items="${listvalue}" var="xx">
                                                <option value="${xx}" <c:if test="${xx == xy.value}">selected</c:if>>
                                                        <%--   ${value}--%>
                                                    <c:if test="${xx=='K10'}">10.000</c:if>
                                                    <c:if test="${xx=='K20'}">20.000</c:if>
                                                    <c:if test="${xx=='K30'}">30.000</c:if>
                ...
                <td>
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" name="cbdelete" id=""
                ...
                <td>
                        <%--                        ${xy.value}--%>
                    <c:if test="${xy.value=='K10'}">10.000</c:if>
                    <c:if test="${xy.value=='K20'}">20.000</c:if>
                    <c:if test="${xy.value=='K30'}">30.000</c:if>
                    <c:if test="${xy.value=='K50'}">50.000</c:if>
                    <c:if test="${xy.value=='K100'}">100.000</c:if>
                    <c:if test="${xy.value=='K200'}">200.000</c:if>
                    <c:if test="${xy.value=='K500'}">500.000</c:if>
                    <c:if test="${xy.value=='K1000'}">1.000.000</c:if>

                </td>
                <td>
                        <%--                        ${xy.classify}--%>
                    <c:if test="${xy.classify=='VT'}">Viettel</c:if>
                    <c:if test="${xy.classify=='VN'}">Vina</c:if>
                    <c:if test="${xy.classify=='MB'}">Mobi</c:if>
                    <c:if test="${xy.classify=='VNMB'}">Vietnammobi</c:if>
                    <c:if test="${xy.classify=='GMB'}">Gmobi</c:if>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <a href="/admin/categori/delete?id=${xy.id}">
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Thêm mới categorie</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form:form action="/admin/categori/add" method="post">
                    <div class="mb-3">
                        <label for="" class="form-label">Tên Sản Phẩm:</label>
                        <input type="text" name="name" id="" class="form-control" placeholder=""
                               aria-describedby="helpId">
...
<nav aria-label="Page navigation example">
    <ul class="pagination justify-content-end">
        <li class="page-item <c:if test="${page.getNumber() < 1 }">disabled</c:if> ">
            ...
            <a class="page-link" href="/admin/categori/getall?pageNumber=${page.getNumber() - 1}"> < </a>
